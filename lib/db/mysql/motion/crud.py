...
async def create_alert(**alert_params: dict[str, ty.Any]) -> None:
    ...
    with db_instance.session() as session:
        session.execute(insert(alerts_table).values(**alert_params))


async def read_alert(uuid: uuid.UUID) -> RowMapping | None:
    """Function to read alert by uuid from db."""
    with db_instance.session() as session:
        res = session.execute(
            select(alerts_table).where(alerts_table.c.uuid == uuid)
        )
