...
class TSL2591:
    ...
    def Lux(self):
        ...
        logging.getLogger().info(f"Lux enabled, resilt: {max(int(lux1), int(0))}")
...
class PhotoEl:
    ...
    def read(self) -> tuple[float, float, float, float]:
        ...
        try:
            ...
            logger.info('Lux: %d'%lux)
            self.tsl2591.TSL2591_SET_LuxInterrupt(50, 200)
            infrared = self.tsl2591.Read_Infrared
            logger.info('Infrared light: %d'%infrared)
            visible = self.tsl2591.Read_Visible
...
ph = PhotoEl(device_name="test", pin=4, voltage=5, on=True)

# Инициализация драйвера MAX7219
spi = busio.SPI(clock=board.SCLK, MISO=None, MOSI=board.MOSI)
cs_pin = digitalio.DigitalInOut(board.CE0)
matrix = adafruit_max7219.matrices.Matrix8x8(spi=spi, cs=cs_pin)

# Входящая переменная: уровень освещенности
illuminance = ctrl.Antecedent(np.arange(0, 5000, 1), 'illuminance')
...
eta_mid_mid = eta
eta_mid_high = eta + 20
eta_high_low = eta_mid_high - 20
eta_high_mid = eta + 250
eta_high_high = 1000
eta_mid_low = eta_mid_mid - 20

low = fuzz.trimf(illuminance.universe, [0, 0, eta_low])
medium = fuzz.trimf(illuminance.universe, [eta_mid_low, eta_mid_mid, eta_mid_high])
high = fuzz.trimf(illuminance.universe, [eta_high_low, eta_high_mid, eta_high_high])


dim = fuzz.trimf(brightness.universe, [0, 0, 20])
normal = fuzz.trimf(brightness.universe, [14, 41, 66])
bright = fuzz.trimf(brightness.universe, [60, 86, 100])

# Функции принадлежности для освещенности (низкая, средняя, высокая)
illuminance['low'] = low
# illuminance['low_medium'] = low_medium
illuminance['medium'] = medium

# illuminance['high_medium'] = high_medium
illuminance['high'] = high

# Функции принадлежности для яркости (низкая, средняя, высокая)
brightness['dim'] = dim
# brightness['normal_dim'] = normal_dim

brightness['normal'] = normal

# brightness['normal_bright'] = normal_bright
brightness['bright'] = bright

# Правила нечёткого вывода
rules = [
    ctrl.Rule(illuminance['low'], brightness['bright']),       # Если темно, делаем ярко
    ctrl.Rule(illuminance['medium'], brightness['normal']),   # Средняя освещенность, нормальный режим
    ctrl.Rule(illuminance['high'], brightness['dim']),         # Высокая освещенность, приглушённый режим
]
...
try:
    ...
    while True:
        lux, _, _, _ = ph.read()
        print(f"Освещенность: {lux:.2f} lux")
        
        # Передача входящего значения освещенности в нечёткий регулятор
        simulator.input['illuminance'] = lux
        simulator.compute()
        print(simulator.output)
        # Получаем выходное значение яркости
        output_brightness = simulator.output['brightness']
        print(f"Яркость матрицы: {output_brightness:.2f}")
        print(round(output_brightness / 100 * 15))
        # Устанавливаем яркость матрицы
        matrix.brightness(round(output_brightness / 100 * 15))  # Масштабируем яркость в диапазон от 0 до 15
        matrix.show()
        time.sleep(1)
    ...
    matrix.brightness(0)  # Выключаем матрицу перед выходом’’’
    matrix.fill(False)